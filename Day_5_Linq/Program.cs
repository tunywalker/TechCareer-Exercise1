...
//TODO:Öyle bir sistem dizayn edin ki ProductDetail listesini dönsün ama categoryId si 2 olan ürünlerin detay
...
//        Stock = p.Stock,
//    }).ToList();
//filterLambdaJoin.ForEach(p => Console.WriteLine(p));


List<int> numbers1 = new List<int>() { 1, 2, 3, 4, 5,-1 };
List<int> numbers2 = new() { 4, 5, 6, 7, 8 };
List<int> numbers3 = new() { 1, 1, 2, 2, 3, 3, 10, 4, 5 };
//TODO: 2 Sayı listesinde ortak sayıları tek liste halinde 
//numbers1.Intersect(numbers2).ToList().ForEach(i => Console.Write((i)+","));
//TODO : 1. Sayı listeisnde bulunan ama2. sayı listesinde bulunmayan sayıların listesi
//numbers1.Except(numbers2).ToList().ForEach(i => Console.Write((i) + ","));

//Console.WriteLine(numbers1.Aggregate((a, b) => a * b));
//Todo: 3. Sayı listesinde bulunan benzersiz sayılar.
//numbers3.Distinct().ToList().ForEach(x => Console.Write((x)+" "));
//Todo: 3 sayı listesini tek liste haline getir
//numbers1.Concat(numbers2).Concat(numbers3).ToList()
//    .ForEach (x => Console.WriteLine(x));
//Todo : Sayı 2 listesinde en büyük ve en küçüç değeri ekrana yazınır.

//Console.WriteLine($"numbers2 listesinde en küçük sayı:{numbers2.Min()}");
//Console.WriteLine($"numbers2 listesinde en büyük sayı:{numbers2.Max()}");

//Todo sayı2 listesinde 2 den büyük 3 tane sayı alan yeni listeyi yazdınırnz.

numbers2.Where(x => x > 2).Take(3).ToList().ForEach(y => Console.WriteLine(y));